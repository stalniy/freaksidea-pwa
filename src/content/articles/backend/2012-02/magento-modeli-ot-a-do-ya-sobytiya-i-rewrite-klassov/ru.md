...
[Из предыдущей статьи](/php_and_somethings/show-62-magento-modeli-ot-a-do-ia-modeli-iadra) стало известно о скоупах _Magento_. Благодаря им переопределять классы можно посредством _copy-paste_ файлов. И потом поменять логику в классе на свое усмотрение. Рассмотрим, что происходить при вызове в коде следующей строки
...
Как приатачить наблюдателя на событие написано [в статье о конфигурации модуля](/php_and_somethings/show-59-magento-konfighuratsiia-ot-a-do-ia-bazovye-nastroiki-modulia).
...
В моделях можно встретить множество событий. К сожалению (или так сразу задумывалось разработчиками) событий в блоках достаточно мало. События в блоках в общем-то сказать и не нужны. Ответ на вопрос почему очень прост. Структура страницы определяется при помощи _layout update_ файлов, о чем было рассказано [в одной из предыдущих глав](/php_and_somethings/show-49-magento-konfighuratsiia-ot-a-do-ia-layout-updates). Существует целых 2 способа, как можно изменить функционал блока не используя события:
