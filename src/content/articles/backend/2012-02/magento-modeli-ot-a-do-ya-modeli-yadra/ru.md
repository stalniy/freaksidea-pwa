...
Класс коллекции **Mage\_Core\_Model\_Mysql4\_Collection\_Abstract** является потомком **Varien\_Data\_Collection\_Db**, который рассматривался [в предыдущей статье](/php_and_somethings/show-61-magento-modeli-ot-a-do-ia-3-kita). Как и ресурс модель он инициализируется при помощи метода **\_init**, который принимает один обязательный параметр _Magento-path_ бизнес модели и второй необязательный _Magento-path_ ресурс модели, если не задан, то второй равен первому.
